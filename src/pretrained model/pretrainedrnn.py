...
class PretrainedRNN:
    ...
    def create_model_variant(self, rnn_layers=2, units_per_layer=[64, 32], 
    ...
    def run_experiments(self):
        ...
        for layers_count in layer_variants:
            ...
            model_path = f'models/rnn_layers_{layers_count}.h5'
            os.makedirs('models', exist_ok=True)
            model.save(model_path)
            
            results[f'rnn_layers_{layers_count}'] = {
                'f1_macro': f1_macro,
        ...
        for i, units in enumerate(units_variants):
            ...
            model_path = f'models/rnn_units_{i+1}.h5'
            model.save(model_path)
        ...
        for bidirectional, name in zip(direction_variants, direction_names):
            ...
            model_path = f'models/rnn_direction_{name.lower()}.h5'
            model.save(model_path)
            ...
            print(f"{name} RNN - Macro F1-Score: {f1_macro:.4f}")
    ...
    def plot_training_curves(self, results):
        ...
        fig.suptitle('RNN Training Curves Comparison')
        
        ax = axes[0]
        for layers_count in [1, 2, 3]:
            key = f'rnn_layers_{layers_count}'
            if key in results:
    ...
    def save_vectorizer(self):
        if self.text_vectorizer is not None:
            ...
            with open('text_vectorizer_vocab.json', 'w') as f:
...
if __name__ == "__main__":
    rnn = PretrainedRNN()
    results = rnn.run_experiments()
    rnn.plot_training_curves(results)
    rnn.save_vectorizer()
    
    print("\n=== RNN Experiments Summary ===")
